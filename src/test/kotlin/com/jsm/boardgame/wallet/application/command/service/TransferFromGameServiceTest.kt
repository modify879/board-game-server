...
private class TransferFromGameFakeWalletRepository : WalletRepository {
...
private class TransferFromGameFakeLedgerEntryRepository : LedgerEntryRepository {
...
class TransferFromGameServiceTest {

    private val wallets = TransferFromGameFakeWalletRepository()
    private val ledger = TransferFromGameFakeLedgerEntryRepository()
    private val clock = Clock.fixed(Instant.parse("2026-01-01T00:00:00Z"), ZoneOffset.UTC)
    private val service = TransferFromGameService(wallets, ledger, clock)
    ...
    fun `캐시아웃이면 잔액이 늘고 GAME_CASH_OUT 엔트리가 balanceAfter 와 함께 남는다`() {
        ...
        assertEquals(LedgerEntryType.GAME_CASH_OUT, entry.type)
        assertEquals(Money.of(5_000), entry.amount)
    ...
    fun `reference 가 GAME_TABLE 타입과 gameTableId 를 담는다`() {
        service.transfer(TransferFromGameCommand(userId = 1, amount = 1_000, gameTableId = 42, memo = null))
    ...
    fun `음수 금액이면 AMOUNT_NEGATIVE`() {
        val e = assertFailsWith<InvalidAmountException> {
            service.transfer(TransferFromGameCommand(userId = 1, amount = -100, gameTableId = 7, memo = null))
        }
    ...
    fun `100원 단위가 아니면 GAME_TRANSFER_AMOUNT_INVALID`() {
        val e = assertFailsWith<InvalidAmountException> {
            service.transfer(TransferFromGameCommand(userId = 1, amount = 1_050, gameTableId = 7, memo = null))
        }
        assertEquals(WalletErrorCode.GAME_TRANSFER_AMOUNT_INVALID, e.errorCode)
    }

    @Test
    fun `지갑이 없던 사용자도 캐시아웃하면 지갑이 lazy 로 만들어지고 잔액이 채워진다`() {
        service.transfer(TransferFromGameCommand(userId = 42, amount = 3_000, gameTableId = 7, memo = null))

        assertEquals(Money.of(3_000), wallets.findByUserId(42)!!.balance)
    }
}
