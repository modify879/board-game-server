...
| `holdem` | 미정 | 홀덤 규칙 전부. 방/좌석을 자기 안에 둔다 |
...
- 홀덤 2~5단계 — 1단계(`wallet` + `ROLE_ADMIN`)까지 끝났다.
  2 `feature/holdem-domain`(카드·핸드 평가·베팅·사이드팟) / 3 `feature/holdem-table`(방·좌석·바이인) /
  4 `feature/holdem-ws`(STOMP) / 5 `feature/holdem-recovery`(스냅샷 복구).
  4단계에서 **브로드캐스트 페이로드 타입에 홀카드 필드를 두지 않는다** — 규칙 6을 컴파일러로 보장한다
- 핸드 히스토리를 남기지 않는다 — 이벤트 소싱이 아니라 진행 중 핸드의 상태 스냅샷 1행
